...
- E.4.2.1 Speex / Nellymoser spec-pinned mono + fixed sample rate — the AudioTagHeader's `SoundRate UB[2]` field can only encode 5.5 / 11 / 22 / 44 kHz, and the `SoundType UB[1]` bit nominally signals mono/stereo, but E.4.2.1 pins three legacy formats independent of those bits: *Speex* is "compressed mono sampled at 16 kHz" (SoundRate "shall be 0", SoundType "shall be 0"); *Nellymoser 8 kHz mono* and *16 kHz mono* (SoundFormat 4 / 5) are mono and the player "ignores the SoundRate and SoundType fields" because the field "cannot represent 8 or 16 kHz". `AudioTagHeader::sample_rate_hz()` now returns 16 kHz for Speex (it previously read the SoundRate idx → 5512 for the spec-mandated idx 0), and `channels()` returns 1 for all three formats regardless of the SoundType bit (it previously read the bit, so a stray stereo bit wrongly reported 2 channels). The `build_audio_stream` `onMetaData` overrides are guarded too: a producer that wrongly stamped `stereo = true` / `audiosamplerate = 44100` for one of these mono-only fixed-rate formats no longer upgrades the stream — the codec id is the authoritative truth, while non-pinned formats (MP3 / AAC / PCM) still pick up the more-authoritative onMetaData values. 1 new `tag::tests::*` unit test (the four-format rate/channel matrix) + 3 new `demuxer::tests::*` unit tests (Speex / Nellymoser-8k stay mono+fixed despite onMetaData; MP3 still honours overrides)
- Annex B.2 `onImageData` embedded-image harvesting + muxer (`FlvDemuxer::embedded_images` + `EmbeddedImage` + `script::write_on_image_data` / `write_on_image_data_body`) — Adobe v10.1 Annex B.2 "Image Metadata": when an F4V/FLV sample is an image type (GIF / PNG / JPEG), the producer surfaces the original compressed image file through an `onImageData` script tag carrying `data` (a BYTEARRAY of the file bytes) + `trackid` (a DOUBLE). The demuxer previously dropped this into the generic unknown-script-name flatten, which collapses a BYTEARRAY to a `bytearray:<len>` length sentinel — data loss for the image bytes. New explicit handling: each `onImageData` seen during discovery parses the `0x11` AVM+ wrapper → AMF3 Object/Array, extracts the `data` ByteArray verbatim and the optional `trackid`, and collects an `EmbeddedImage { track_id, data }` into a dedicated side-channel exposed via `FlvDemuxer::embedded_images()` (the binary bytes have no representation in the string `metadata()` bag). The bag still gains `onimagedata.<n>.trackid` / `.length` scalars so a caller scanning only the bag sees an image is present. Write side: `script::write_on_image_data(w, ts, image_data, track_id)` emits the spec-conforming `onImageData` script tag (AMF0 name → AVM+ marker → AMF3 anonymous dynamic object with the `data` ByteArray + optional `trackid` Double) — a BYTEARRAY has no AMF0 form so the AMF3-behind-AVM+ carriage is required. 3 new `script::tests::*` unit tests (AVM+ ByteArray body, trackid-omitted, full tag framing) + 2 new `tests/roundtrip_muxer.rs` integration tests (single JPEG round-trip with bag scalars + multi-image discovery-order with a trackid-less image). `EmbeddedImage`, `write_on_image_data` / `write_on_image_data_body` re-exported from the crate root
- Annex B.1 Flash Player Metadata typed accessors (`TypedMetadata::avc_level` / `avc_profile` / `moov_position` / `video_codec_fourcc` / `audio_codec_fourcc`) — the v10.1 Annex B.1 "Stream Properties" table documents the property set Flash Player surfaces on `NetStream.onMetaData`; several of those (the AVC level/profile, the moov-box offset, the four-character codec STRING forms) have no Annex E.5 equivalent and so had no typed read view, surfacing only as raw bag strings. New accessors re-type them: `avc_level()` / `avc_profile()` return the finite Number (DOUBLE) `level_idc` / `profile_idc` a producer stamps when the video is AVC-coded; `moov_position()` returns the absolute F4V `moov`-box byte offset (gated finite + non-negative, as `u64`); `video_codec_fourcc()` / `audio_codec_fourcc()` return the Annex B.1 four-character codec STRING (`"avc1"` / `"mp4a"` / …) — distinct from the Annex E.5 Number form `video_codec_id_str` / `audio_codec_id_str` resolve — disambiguated by whether the stored value parses as a number (numeric → Annex E.5 form, `None` here; non-numeric → the String form, returned verbatim). 6 new `typed_meta::tests::annex_b1_*` unit tests cover the populated / absent / malformed / numeric-vs-String cases
- AMF0 Long String writer (`amf0::write_long_string` / `write_string_auto`) — the demuxer parsed the AMF0 Long String type (`0x0C`, UI32 length, §2.13) into `AmfValue::String`, but the muxer only had the ordinary String writer (`0x02`, UI16 length) capped at 65535 bytes, so it could not emit a string past that ceiling. New `write_long_string` emits the UI32-length form; `write_string_auto` picks the narrowest spec-valid encoding (ordinary String when ≤ 65535 bytes, else Long String) matching what a conforming producer writes. `script::write_on_xmp_data_body` now routes `liveXML` through `write_string_auto`, so XMP packets larger than 64 KB (routine for real XMP envelopes) round-trip through `FlvDemuxer` under `metadata["xmp"]` instead of overflowing the UI16 length. Both forms parse back into `AmfValue::String` (indistinguishable post-parse — only the wire encoding differs). 2 new `amf0::tests::*` unit tests (long-string parse-back + auto narrowest-encoding incl. the 65535-byte boundary) + 1 new `tests/roundtrip_muxer.rs` integration test (~85 KB XMP packet survives verbatim through the demuxer). Removes the prior "Long String path left to callers" caveat on `write_on_xmp_data`
- `|AdditionalHeader` Encryption Header muxer (`script::write_additional_header` + `script::EncryptionHeader`) — the write-side inverse of the demuxer's `parse_additional_header`, completing the encrypted-FLV head together with the filtered-tag muxer. In an encrypted FLV the `|AdditionalHeader` SCRIPTDATA object (spec Annex F.2.1, timestamp 0, immediately after `onMetaData`) carries the Encryption Header so a DRM client can fetch the key before any filtered tag. New `EncryptionHeader` types the fields the demuxer surfaces as `encryption.*` metadata — `Version` (1 = FMRMS v1.x / 2 = Flash Access 2.0), `Method` ("Standard"), `EncryptionAlgorithm` ("AES-CBC"), `KeyLength` (16), `KeyInfo.SubType` ("APS" v1 / "FlashAccessv2" v2) — with `flash_access_v2()` / `fmrms_v1()` conventional constructors. `write_additional_header_body` emits the exact nested AMF0 object shape (`{Encryption: {Version, Method, Flags, Params: {Version, EncryptionAlgorithm, EncryptionParams: {KeyLength}, KeyInfo: {SubType}}}}`) that `parse_additional_header` walks; `write_additional_header` wraps it in the timestamp-0 ScriptData tag. 2 new `script::tests::*` unit tests (v2 nested-object parse-back + v1 APS subtype) + 1 new `tests/roundtrip_muxer.rs` integration test driving a complete encrypted-FLV head (`onMetaData` + `|AdditionalHeader` + SequenceStart + fully-encrypted video tag) through `FlvDemuxer`: `is_encrypted()` flips true and all five `encryption.*` keys read back. `write_additional_header` / `write_additional_header_body` / `EncryptionHeader` re-exported from the crate root
- Filtered (encrypted) tag muxer (`tag::write_filtered_tag` + `EncryptedTagPreamble::{encryption, selective, to_bytes}`) — the demuxer parsed the Annex F.3 `EncryptionTagHeader` + `FilterParams` preamble off filtered tags, but there was no write-side inverse, so a DRM-protected FLV could be demuxed but not re-muxed. New `EncryptedTagPreamble::to_bytes` is the exact byte-for-byte inverse of `parse` (NumFilters=1 + NUL-terminated FilterName + UI24 Length + FilterParams), with `encryption(iv)` (non-selective version 1 — IV-only `EncryptionFilterParams`) and `selective(Option<iv>)` (version 2 `SE` — `EncryptedAU` UB[1] + Reserved UB[7] + optional IV) constructors. New `write_filtered_tag` writes a full filtered tag: tag header with the `Filter` bit (`0x20`) OR'd onto the tag type, the preamble, the (cipher)text body, and the trailing `PreviousTagSize`. The crate carries no DRM client — `body` is whatever the caller's filter chain produced — so this closes the *container* loop (protected-tag framing round-trips) without implying decryption. `to_bytes` rejects a missing IV (full encryption / `EncryptedAU=1`) and an unknown FilterName before emitting any bytes. 5 new `tag::tests::*` unit tests (v1 / SE-encrypted / SE-cleartext round-trips, missing-IV rejection, full filtered-tag framing) + 2 new `tests/roundtrip_muxer.rs` integration tests (full "Encryption" video tag + SE-cleartext audio tag both strip-and-survive through `FlvDemuxer`, body verbatim, discard-flagged). `write_filtered_tag` re-exported from the crate root
- Multitrack write → demux side-channel round-trip (end-to-end `join_tracks` parity) — the existing multitrack mux→demux tests hand-rolled the `[FourCc?] trackId UI8 sizeOfTrack UI24 payload` body bytes and asserted only the default track on the trait-object packet. New integration tests close the write-side loop using the documented `join_tracks` serialiser (the inverse of `split_tracks`): a `Vec<JoinTrack>` is serialised, threaded through `write_ex_video_tag` / `write_ex_audio_tag`, demuxed via the concrete `FlvDemuxer`, and EVERY track recovered through `last_multitrack_tracks()` — per-track `trackId`, `fourcc`, resolved `codec_name`, and payload all survive end to end. Covers `ManyTracks` (3 shared-codec AV1 variants) and `ManyTracksManyCodecs` on both pipelines (video AV1+VP9, audio Opus+FLAC), so the per-track FourCc recovery (`Ex` header `fourcc: None`, each track carrying its own makeFourCc) is observable rather than inferred from a single shared codec. 3 new `tests/roundtrip_muxer.rs` integration tests. `join_tracks` / `JoinTrack` re-exported from the crate root were already public; these are the first end-to-end `join_tracks` → `FlvDemuxer` round-trips
